...
const multi = people.filter(p => p.parent_edges.length > 1);
let shown = 0;
for (const p of multi) {
  const res = ageAllEdges(chart, p.birth?.text, p.parent_edges,
                          e => idx.get(e.parent)?.birth?.text);
  if (!res.ruledOut || res.left !== 1 || shown >= 3) continue;
  shown++;
  console.log(`\n${p.name}（第${p.gen}世）${p.src_human}`);
  console.log(`  谱上写父名「${p.father_name}」，同名 ${p.parent_edges.length} 个：`);
